...
void compileForSt(void)
{
  ...
  eat(SB_ASSIGN);
  Type *initType = compileExpression();
  checkIntType(initType);

  eat(KW_TO);
  Type *endType = compileExpression();
...
void compileCondition(void)
{
  ...
  Type *lshType = compileExpression();
  Type *rshType;

  checkBasicType(lshType);
  ...
  rshType = compileExpression();
  checkBasicType(rshType);
  checkTypeEquality(lshType, rshType);
}
...
Type *compileIndexes(Type *arrayType)
{
  ...
  while (lookAhead->tokenType == SB_LSEL)
  {
    ...
    Type *indexType = compileExpression();
